...
if [ -f /etc/log-create-shadowsocks.log ]; then
    mapfile -t USERS < <(grep "^Remarks" /etc/log-create-shadowsocks.log | awk '{print $3}' | sort -u)
else
    mapfile -t USERS < <(grep '^### ' /etc/xray/config.json | awk '{print $2}' | sort -u)
fi
echo "--------------------------------------------"
echo "-------=[ Shadowsocks User Login - LIVE ]=--"
echo "--------------------------------------------"
echo -e " ${YELLOW}Koneksi aktif 60 detik terakhir${NC}"
echo "--------------------------------------------"
if [ ${#USERS[@]} -eq 0 ]; then
    echo -e " ${RED}Tidak ada akun Shadowsocks di config${NC}"; echo "--------------------------------------------"
...
[ $FOUND_ANY -eq 0 ] && echo -e " ${RED}Tidak ada user Shadowsocks yang terkoneksi${NC}" && echo "--------------------------------------------"
